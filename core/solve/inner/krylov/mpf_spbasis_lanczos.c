...
)
{
  ...
  if (solver->data_type == MPF_REAL)
  {
    ...
    solver->inner_function = &mpf_dsy_spbasis_lanczos;
  ...
  )
  {
    ...
    solver->inner_function = &mpf_zsy_spbasis_lanczos;
    solver->device = MPF_DEVICE_CPU;
  }
  else if ((solver->data_type == MPF_COMPLEX)
          && (solver->matrix_type == MPF_MATRIX_HERMITIAN))
  {
    ...
    solver->inner_function = &mpf_zhe_spbasis_lanczos;
    solver->device = MPF_DEVICE_CPU;
  }
...
/*
    only 3 vectors are required to be stored at a time
        * 2 of them are the previous
        * 1 of them is the newly computed

    vcurr
    vprev
    w (next)

    the rest off the basis can be stored as a sparsified basis. */
...
)
{
  ...
  MPF_Int offset = solver->current_rhs%solver->max_blk_fA;
  ...
  for (MPF_Int k = 0; k < outer_iterations; ++k)
  {
    ...
    for (MPF_Int j = 1; j < inner_iterations; ++j)
    {
      ...
      end_sparse += solver->color_to_node_map.bins_size[color];    // better max error, this is the correct one
    }

    /* solves linear system of equations and checks termination condition */
    mpf_qr_dsy_givens(m_H, n_H, 1, H, ld_H, br); // true
    mpf_dtrsm(CblasColMajor, CblasLeft, CblasUpper, CblasNoTrans, CblasNonUnit,
      m_H, 1, 1.0, H, n_H, br, m_H);
    krylov_dge_sparse_basis_combine(
      solver->color_to_node_map.bins_size[color],/* num_rows_V */
      n_H,               /* num_cols_V */
      V,                 /* sparsified V matrix */
      &solver->color_to_node_map, /* color to node map (1 to m) */
      solver->max_blk_fA,
      color,             /* current color */
      offset,            /* current offset */
      br,                /* coefficients vector */
      x,                 /* X vector */
      m_B);              /* num_rows_X */
...
)
{
  ...
  /* required for sparsified solvers */
  MPF_Int offset = solver->current_rhs%solver->max_blk_fA;
  MPF_Int color = solver->current_rhs/solver->max_blk_fA;
  MPF_Int end_sparse = 0;
  ...
  for (MPF_Int k = 0; k < outer_iterations; ++k)
  {
    ...
    for (MPF_Int j = 1; j < inner_iterations; ++j)
    {
      ...
      end_sparse += solver->color_to_node_map.bins_size[color];
    ...
    krylov_zge_sparse_basis_combine
    (
      solver->color_to_node_map.bins_size[color],   /* num_rows_V */
      n_H,                                   /* num_cols_V */
      V,                                     /* sparsified V matrix */
...
)
{
  ...
  for (MPF_Int k = 0; k < outer_iterations; ++k)
  {
    ...
    mpf_sparse_z_mv(SPARSE_OPERATION_NON_TRANSPOSE, ONE_C, A->handle,
      A->descr, V, ZERO_C, w);
    mpf_zgemm(CblasColMajor, CblasTrans, CblasNoTrans, 1, 1, m_B, &ONE_C, w,
      m_B, V, m_B, &ZERO_C, &H[0], m_B);
    tempf_complex = mpf_scalar_z_invert_sign(H[0]);
    ...
    for (MPF_Int j = 1; j < inner_iterations; ++j)
    {
      ...
      mpf_zgemm(CblasColMajor, CblasTrans, CblasNoTrans, 1, 1, m_B,
         &ONE_C, w, m_B, w, m_B, &ZERO_C, &tempf_complex, m_B);
    ...
    /* termination condition */
    mpf_qr_zsy_givens_2(H, br, ld_H, n_H, blk);  // note: uses this in order to avoid error when ld_H != m_H
    mpf_ztrsm(CblasColMajor, CblasLeft, CblasUpper, CblasNoTrans,
      CblasNonUnit, m_H, 1, &ONE_C, H, m_H, br, m_H);
    ...
    memcpy(r, b, (sizeof*r)*m_B);
    mpf_sparse_z_mv(SPARSE_OPERATION_NON_TRANSPOSE, COMPLEX_MINUS_ONE,
      A->handle, A->descr, x, ONE_C, r);
    r_norm = mpf_dznrm2(m_B, r, 1);
    ...
    {
      mpf_zgemm(CblasColMajor, CblasTrans, CblasNoTrans, 1, 1, m_B,
        &ONE_C, r, m_B, r, m_B, &ZERO_C, &tempf_complex, 1);
      mpf_vectorized_z_sqrt(1, &tempf_complex, &tempf_complex);
      memcpy(V, r, (sizeof *V) * m_B);
